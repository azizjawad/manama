...
class ProductController extends Controller
{
    ...
    public function category_product($category_slug, $product_slug = "")
    {
        ...
        if (isset($data['category']->id)) {
            if (empty($product_slug)) {
                ...
                $data = array_merge($data, self::get_product($data['category']->id, $product_slug));
    ...
    private function get_category_products($category_id, $limit = '', $exclude_id = ''){
        ...
        $query = ProductsModel::select(['products.image as product_image','products.page_slug as product_slug',
            'products.label','product_info.listing_name as product_name','product_info.packaging_weight',
    ...
    private function get_product($category_id, $product_slug): array
    {
        $related_products = [];
        $gallery = [];

        $products =  ProductsModel::select(['products.*','product_info.listing_name as product_name','product_info.packaging_weight',
            'product_info.packaging_type','product_info.cost_price','product_info.sku_code','product_info.barcode'])
            ->join('product_info','product_info.product_id','products.id')
            ->where('products.category_id',$category_id)
